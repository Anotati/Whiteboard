...
import { degreesToRadians, lineSegment, pointFrom, pointDistance, pointFromArray, pointRotateRads, } from "../../math";
export class ElementBounds {
    ...
    static calculateBounds(element, elementsMap) {
        ...
        if (isFreeDrawElement(element)) {
            const [minX, minY, maxX, maxY] = getBoundsFromPoints(element.points.map(([x, y]) => pointRotateRads(pointFrom(x, y), pointFrom(cx - element.x, cy - element.y), element.angle)));
            return [
                minX + element.x,
                minY + element.y,
                maxX + element.x,
...
export const getElementAbsoluteCoords = (element, elementsMap, includeBoundText = false) => {
    if (isFreeDrawElement(element)) {
        ...
        if (isArrowElement(container)) {
            const { x, y } = LinearElementEditor.getBoundTextElementPosition(container, element, elementsMap);
            return [
                x,
                y,
                x + element.width,
...
export const getElementLineSegments = (element, elementsMap) => {
    ...
    if (isLinearElement(element) || isFreeDrawElement(element)) {
        ...
        while (i < element.points.length - 1) {
            segments.push(lineSegment(pointRotateRads(pointFrom(element.points[i][0] + element.x, element.points[i][1] + element.y), center, element.angle), pointRotateRads(pointFrom(element.points[i + 1][0] + element.x, element.points[i + 1][1] + element.y), center, element.angle)));
            i++;
        }
        return segments;
    }
...
export const getArrowheadPoints = (element, shape, position, arrowhead) => {
    ...
    if (arrowhead === "diamond" || arrowhead === "diamond_outline") {
        ...
        if (position === "start") {
            ...
            [ox, oy] = pointRotateRads(pointFrom(x2 + minSize * 2, y2), pointFrom(x2, y2), Math.atan2(py - y2, px - x2));
        }
        else {
            const [px, py] = element.points.length > 1
                ? element.points[element.points.length - 2]
                : [0, 0];
            [ox, oy] = pointRotateRads(pointFrom(x2 - minSize * 2, y2), pointFrom(x2, y2), Math.atan2(y2 - py, x2 - px));
        }
...
const getLinearElementRotatedBounds = (element, cx, cy, elementsMap) => {
    ...
    if (element.points.length < 2) {
        ...
        const [x, y] = pointRotateRads(pointFrom(element.x + pointX, element.y + pointY), pointFrom(cx, cy), element.angle);
        let coords = [x, y, x, y];
        if (boundTextElement) {
    ...
    const transformXY = ([x, y]) => pointRotateRads(pointFrom(element.x + x, element.y + y), pointFrom(cx, cy), element.angle);
...
export const getCommonBounds = (elements, elementsMap) => {
    if (!elements.length) {
        return [0, 0, 0, 0];
    }
    let minX = Infinity;
...
export const getClosestElementBounds = (elements, from) => {
    ...
    elements.forEach((element) => {
        ...
        const distance = pointDistance(pointFrom((x1 + x2) / 2, (y1 + y2) / 2), pointFrom(from.x, from.y));
...
export const getCenterForBounds = (bounds) => pointFrom(bounds[0] + (bounds[2] - bounds[0]) / 2, bounds[1] + (bounds[3] - bounds[1]) / 2);
