...
export class LinearElementEditor {
    ...
    static handlePointDragging(event, app, scenePointerX, scenePointerY, maybeSuggestBinding, linearElementEditor, scene) {
        ...
        if (selectedPointsIndices && draggingPoint) {
            if (shouldRotateWithDiscreteAngle(event) &&
                ...
                const [width, height] = LinearElementEditor._getShiftLockedDelta(element, elementsMap, referencePoint, pointFrom(scenePointerX, scenePointerY), event[KEYS.CTRL_OR_CMD] ? null : app.getEffectiveGridSize());
                ...
                LinearElementEditor.movePoints(element, selectedPointsIndices.map((pointIndex) => {
                    const newPointPosition = pointIndex === lastClickedPoint
                        ? LinearElementEditor.createPointAt(element, elementsMap, scenePointerX - linearElementEditor.pointerOffset.x, scenePointerY - linearElementEditor.pointerOffset.y, event[KEYS.CTRL_OR_CMD] ? null : app.getEffectiveGridSize())
                        : pointFrom(element.points[pointIndex][0] + deltaX, element.points[pointIndex][1] + deltaY);
                    return {
                        index: pointIndex,
    ...
    static getSegmentMidPoint(element, startPoint, endPoint, endPointIndex, elementsMap) {
        ...
        if (element.points.length > 2 && element.roundness) {
            ...
            if (controlPoints) {
                ...
                segmentMidPoint = LinearElementEditor.getPointGlobalCoordinates(element, getBezierXY(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], t), elementsMap);
            }
        }
    ...
    static handlePointerDown(event, app, store, scenePointer, linearElementEditor, scene) {
        ...
        if (event.altKey && appState.editingLinearElement) {
            if (linearElementEditor.lastUncommittedPoint == null &&
                ...
                mutateElement(element, {
                    points: [
                        ...element.points,
                        LinearElementEditor.createPointAt(element, elementsMap, scenePointer.x, scenePointer.y, event[KEYS.CTRL_OR_CMD] ? null : app.getEffectiveGridSize()),
                    ],
                });
        ...
        const targetPoint = clickedPointIndex > -1 &&
            pointRotateRads(pointFrom(element.x + element.points[clickedPointIndex][0], element.y + element.points[clickedPointIndex][1]), pointFrom(cx, cy), element.angle);
    ...
    static handlePointerMove(event, scenePointerX, scenePointerY, app, elementsMap) {
        ...
        if (shouldRotateWithDiscreteAngle(event) && points.length >= 2) {
            ...
            const [width, height] = LinearElementEditor._getShiftLockedDelta(element, elementsMap, lastCommittedPoint, pointFrom(scenePointerX, scenePointerY), event[KEYS.CTRL_OR_CMD] ? null : app.getEffectiveGridSize());
            newPoint = pointFrom(width + lastCommittedPoint[0], height + lastCommittedPoint[1]);
        }
        else {
    ...
    elementsMap) {
        ...
        const { x, y } = element;
        return p
            ? pointRotateRads(pointFrom(x + p[0], y + p[1]), pointFrom(cx, cy), element.angle)
    ...
    static pointFromAbsoluteCoords(element, absoluteCoords, elementsMap) {
        ...
        const cx = (x1 + x2) / 2;
        const cy = (y1 + y2) / 2;
        const [x, y] = pointRotateRads(pointFrom(absoluteCoords[0], absoluteCoords[1]), pointFrom(cx, cy), -element.angle);
    ...
    static getPointIndexUnderCursor(element, elementsMap, zoom, x, y) {
        ...
        while (--idx > -1) {
            ...
            if (pointDistance(pointFrom(x, y), pointFrom(p[0], p[1])) * zoom.value <
    ...
    static createPointAt(element, elementsMap, scenePointerX, scenePointerY, gridSize) {
        ...
        const cx = (x1 + x2) / 2;
        const cy = (y1 + y2) / 2;
        const [rotatedX, rotatedY] = pointRotateRads(pointFrom(pointerOnGrid[0], pointerOnGrid[1]), pointFrom(cx, cy), -element.angle);
    ...
    static duplicateSelectedPoints(appState, elementsMap) {
        invariant(appState.editingLinearElement, "Not currently editing a linear element");
    ...
    static shouldAddMidpoint(linearElementEditor, pointerCoords, appState, elementsMap) {
        ...
        const dist = pointDistance(pointFrom(origin.x, origin.y), pointFrom(pointerCoords.x, pointerCoords.y));
        if (!appState.editingLinearElement &&
    ...
    static addMidpoint(linearElementEditor, pointerCoords, app, snapToGrid, elementsMap) {
        ...
        const midpoint = LinearElementEditor.createPointAt(element, elementsMap, pointerCoords.x, pointerCoords.y, snapToGrid && !isElbowArrow(element) ? app.getEffectiveGridSize() : null);
        const points = [
    ...
    static _updatePoints(element, nextPoints, offsetX, offsetY, elementsMap, otherUpdates, options) {
        if (isElbowArrow(element)) {
            ...
            const rotated = pointRotateRads(pointFrom(offsetX, offsetY), pointFrom(dX, dY), element.angle);
            mutateElement(element, {
                ...otherUpdates,
                points: nextPoints,
    ...
    static _getShiftLockedDelta(element, elementsMap, referencePoint, scenePointer, gridSize) {
        ...
        return pointRotateRads(pointFrom(width, height), pointFrom(0, 0), -element.angle);
    }
    ...
    static getMinMaxXYWithBoundText = (element, elementsMap, elementBounds, boundTextElement) => {
        ...
        const counterRotateBoundTextTopLeft = pointRotateRads(pointFrom(boundTextX1, boundTextY1), centerPoint, -element.angle);
        const counterRotateBoundTextTopRight = pointRotateRads(pointFrom(boundTextX2, boundTextY1), centerPoint, -element.angle);
        const counterRotateBoundTextBottomLeft = pointRotateRads(pointFrom(boundTextX1, boundTextY2), centerPoint, -element.angle);
        const counterRotateBoundTextBottomRight = pointRotateRads(pointFrom(boundTextX2, boundTextY2), centerPoint, -element.angle);
