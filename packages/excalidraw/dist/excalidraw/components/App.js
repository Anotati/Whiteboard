...
import { APP_NAME, CURSOR_TYPE, DEFAULT_MAX_IMAGE_WIDTH_OR_HEIGHT, DEFAULT_VERTICAL_ALIGN, DRAGGING_THRESHOLD, ELEMENT_SHIFT_TRANSLATE_AMOUNT, ELEMENT_TRANSLATE_AMOUNT, ENV, EVENT, FRAME_STYLE, IMAGE_MIME_TYPES, IMAGE_RENDER_TIMEOUT, isBrave, LINE_CONFIRM_THRESHOLD, MAX_ALLOWED_FILE_BYTES, MIME_TYPES, MQ_MAX_HEIGHT_LANDSCAPE, MQ_MAX_WIDTH_LANDSCAPE, MQ_MAX_WIDTH_PORTRAIT, MQ_RIGHT_SIDEBAR_MIN_WIDTH, POINTER_BUTTON, ROUNDNESS, SCROLL_TIMEOUT, TAP_TWICE_TIMEOUT, TEXT_TO_CENTER_SNAP_THRESHOLD, THEME, THEME_FILTER, TOUCH_CTX_MENU_TIMEOUT, VERTICAL_ALIGN, YOUTUBE_STATES, ZOOM_STEP, POINTER_EVENTS, TOOL_TYPE, isIOS, supportsResizeObserver, DEFAULT_COLLISION_THRESHOLD, DEFAULT_TEXT_ALIGN, } from "../constants";
...
import { dataURLToFile, generateIdFromFile, getDataURL, getFileFromEvent, ImageURLToFile, isImageFileHandle, isSupportedImageFile, loadSceneOrLibraryFromBlob, normalizeFile, parseLibraryJSON, resizeImageFile, SVGStringToFile, } from "../data/blob";
...
import { bindTextToShapeAfterDuplication, getApproxMinLineHeight, getApproxMinLineWidth, getBoundTextElement, getContainerCenter, getContainerElement, getLineHeightInPx, getMinTextElementWidth, isMeasureTextSupported, isValidTextContainer, measureText, normalizeText, wrapText, } from "../element/textElement";
...
export const ExcalidrawContainerContext = React.createContext({ container: null, id: null });
...
const ExcalidrawActionManagerContext = React.createContext(null);
...
class App extends React.Component {
    ...
    handleEmbeddableCenterClick(element) {
        if (this.state.activeEmbeddable?.element === element &&
    ...
    renderEmbeddables() {
        ...
        return (_jsx(_Fragment, { children: embeddableElements.map((el) => {
                ...
                const isHovered = this.state.activeEmbeddable?.element === el &&
                    this.state.activeEmbeddable?.state === "hover";
                return (_jsx("div", { className: clsx("excalidraw__embeddable-container", {
                        ...
                        opacity: getRenderOpacity(el, getContainingFrame(el, this.scene.getNonDeletedElementsMap()), this.elementsPendingErasure, null),
                        ...
                        }, children: [isHovered && (_jsx("div", { className: "excalidraw__embeddable-hint", children: t("buttons.embeddableInteractionButton") })), _jsx("div", { className: "excalidraw__embeddable__outer", style: {
                                    padding: `${el.strokeWidth}px`,
                                }, children: (isEmbeddableElement(el)
                                    ? this.props.renderEmbeddable?.(el, this.state)
                                    : null) ?? (_jsx("iframe", { ref: (ref) => this.cacheEmbeddableRef(el, ref), className: "excalidraw__embeddable", srcDoc: src?.type === "document"
        ...
        get: (frameElement) => {
            ...
            if (!bounds ||
                ...
                if (frameNameDiv) {
                    ...
                    const boxSceneBottomRight = viewportCoordsToSceneCoords({ clientX: box.right, clientY: box.bottom }, this.state);
                    bounds = {
                        x: boxSceneTopLeft.x,
                        y: boxSceneTopLeft.y,
    ...
    renderFrameNames = () => {
        ...
        return this.scene.getNonDeletedFramesLikes().map((f) => {
            ...
            return (_jsx("div", { id: this.getFrameNameDOMId(f), style: {
                    position: "absolute",
                    ...
                    : POINTER_EVENTS.enabled,
                                                        ...
                                                        firstSelectedElement.customData?.generationData
                                                            ?.status === "done" && (_jsxs(ElementCanvasButtons, { element: firstSelectedElement, elementsMap: elementsMap, children: [_jsx(ElementCanvasButton, { title: t("labels.copySource"), icon: copyIcon, checked: false, onChange: () => this.onIframeSrcCopy(firstSelectedElement) }), _jsx(ElementCanvasButton, { title: "Enter fullscreen", icon: fullscreenIcon, checked: false, onChange: () => {
                                                                ...
                                                                } })] })), this.state.toast !== null && (_jsx(Toast, { message: this.state.toast.message, onClose: () => this.setToast(null), duration: this.state.toast.duration, closable: this.state.toast.closable })), this.state.contextMenu && (_jsx(ContextMenu, { items: this.state.contextMenu.items, top: this.state.contextMenu.top, left: this.state.contextMenu.left, actionManager: this.actionManager, onClose: (callback) => {
                                                        ...
                                                        } }), this.state.newElement && (_jsx(NewElementCanvas, { appState: this.state, scale: window.devicePixelRatio, rc: this.rc, elementsMap: elementsMap, allElementsMap: allElementsMap, renderConfig: {
                                                            imageCache: this.imageCache,
                                                            isExporting: false,
                                                            renderGrid: false,
                                                            canvasBackgroundColor: this.state.viewBackgroundColor,
    ...
    updateMagicGeneration = ({ frameElement, data, }) => {
        if (data.status === "pending") {
            ...
            mutateElement(frameElement, { customData: { generationData: undefined } }, false);
        }
        else {
     ...
     */
    resetScene = withBatchedUpdates((opts) => {
    ...
    async componentDidMount() {
        ...
        if (isBrave() && !isMeasureTextSupported()) {
            this.setState({
                errorMessage: _jsx(BraveMeasureTextError, {}),
            });
        }
    }
    ...
    addEventListeners() {
        ...
        this.onRemoveEventListenersEmitter.once(addEventListener(this.excalidrawContainerRef.current, EVENT.WHEEL, this.handleWheel, { passive: false }), addEventListener(window, EVENT.MESSAGE, this.onWindowMessage, false), addEventListener(document, EVENT.POINTER_UP, this.removePointer), // #3553
        ...
        // -------------------------------------------------------------------------
        this.onRemoveEventListenersEmitter.once(addEventListener(document, EVENT.FULLSCREENCHANGE, this.onFullscreenChange), addEventListener(document, EVENT.PASTE, this.pasteFromClipboard), addEventListener(document, EVENT.CUT, this.onCut), addEventListener(window, EVENT.RESIZE, this.onResize, false), addEventListener(window, EVENT.UNLOAD, this.onUnload, false), addEventListener(window, EVENT.BLUR, this.onBlur, false), addEventListener(this.excalidrawContainerRef.current, EVENT.WHEEL, this.handleWheel), addEventListener(this.excalidrawContainerRef.current, EVENT.DRAG_OVER, this.disableEvent, false), addEventListener(this.excalidrawContainerRef.current, EVENT.DROP, this.disableEvent, false));
    ...
    pasteFromClipboard = withBatchedUpdates(async (event) => {
    ...
    toggleLock = (source = "ui") => {
        if (!this.state.activeTool.locked) {
            trackEvent("toolbar", "toggleLock", `${source} (${this.device.editor.isMobile ? "mobile" : "desktop"})`);
        }
        this.setState((prevState) => {
            return {
                activeTool: {
                    ...prevState.activeTool,
                    ...updateActiveTool(this.state, prevState.activeTool.locked
                        ? { type: "selection" }
                        : prevState.activeTool),
    ...
    addFiles = withBatchedUpdates((files) => {
    ...
    updateCurrentCursorPosition = withBatchedUpdates((event) => {
    ...
    // Input handling
    onKeyDown = withBatchedUpdates((event) => {
        ...
        if (!isInputLike(event.target)) {
            ...
            if (event[KEYS.CTRL_OR_CMD] && arrowKeyPressed && !event.shiftKey) {
                event.preventDefault();
                const selectedElements = getSelectedElements(this.scene.getNonDeletedElementsMap(), this.state);
                if (selectedElements.length === 1 &&
                    isFlowchartNodeElement(selectedElements[0])) {
                    this.flowChartCreator.createNodes(selectedElements[0], this.scene.getNonDeletedElementsMap(), this.state, getLinkDirectionFromKey(event.key));
                }
                if (this.flowChartCreator.pendingNodes?.length &&
                    !isElementCompletelyInViewport(this.flowChartCreator.pendingNodes, this.canvas.width / window.devicePixelRatio, this.canvas.height / window.devicePixelRatio, {
                        offsetLeft: this.state.offsetLeft,
                        offsetTop: this.state.offsetTop,
                        scrollX: this.state.scrollX,
                        scrollY: this.state.scrollY,
            ...
            if (event.altKey) {
                ...
                if (selectedElements.length === 1 && arrowKeyPressed) {
                    event.preventDefault();
                    const nextId = this.flowChartNavigator.exploreByDirection(selectedElements[0], this.scene.getNonDeletedElementsMap(), getLinkDirectionFromKey(event.key));
                    if (nextId) {
                        this.setState((prevState) => ({
                            selectedElementIds: makeNextSelectedElementIds({
                                [nextId]: true,
                        ...
                        const nextNode = this.scene
                            ...
                            .get(nextId);
                        if (nextNode &&
                            !isElementCompletelyInViewport([nextNode], this.canvas.width / window.devicePixelRatio, this.canvas.height / window.devicePixelRatio, {
                                offsetLeft: this.state.offsetLeft,
                                offsetTop: this.state.offsetTop,
                                scrollX: this.state.scrollX,
                                scrollY: this.state.scrollY,
        ...
        else if (event.key.toLowerCase() === KEYS.E &&
            event.shiftKey &&
        ...
        if (!event[KEYS.CTRL_OR_CMD]) {
            if (this.flowChartCreator.isCreatingChart) {
                ...
                if (firstNode) {
                    ...
                    if (!isElementCompletelyInViewport([firstNode], this.canvas.width / window.devicePixelRatio, this.canvas.height / window.devicePixelRatio, {
                        offsetLeft: this.state.offsetLeft,
                        offsetTop: this.state.offsetTop,
                        scrollX: this.state.scrollX,
                        scrollY: this.state.scrollY,
    ...
    getElementLinkAtPosition = (scenePointer, hitElement) => {
        ...
        return elements.find((element, index) => {
            ...
            return (element.link &&
                ...
                isPointHittingLink(element, this.scene.getNonDeletedElementsMap(), this.state, pointFrom(scenePointer.x, scenePointer.y), this.device.editor.isMobile));
        });
    };
    redirectToLink = (event, isTouchScreen) => {
        const draggedDistance = pointDistance(pointFrom(this.lastPointerDownEvent.clientX, this.lastPointerDownEvent.clientY), pointFrom(this.lastPointerUpEvent.clientX, this.lastPointerUpEvent.clientY));
        ...
        const elementsMap = this.scene.getNonDeletedElementsMap();
        const lastPointerDownHittingLinkIcon = isPointHittingLink(this.hitLinkElement, elementsMap, this.state, pointFrom(lastPointerDownCoords.x, lastPointerDownCoords.y), this.device.editor.isMobile);
        const lastPointerUpCoords = viewportCoordsToSceneCoords(this.lastPointerUpEvent, this.state);
        const lastPointerUpHittingLinkIcon = isPointHittingLink(this.hitLinkElement, elementsMap, this.state, pointFrom(lastPointerUpCoords.x, lastPointerUpCoords.y), this.device.editor.isMobile);
    ...
    handleCanvasPointerMove = (event) => {
        ...
        if (!this.state.newElement &&
            ...
            const { originOffset, snapLines } = getSnapLinesAtPointer(this.scene.getNonDeletedElements(), this, {
                x: scenePointerX,
                y: scenePointerY,
        ...
        if (this.state.editingLinearElement &&
            ...
            const editingLinearElement = LinearElementEditor.handlePointerMove(event, scenePointerX, scenePointerY, this, this.scene.getNonDeletedElementsMap());
        ...
        if (this.state.multiElement) {
            ...
            if (lastPoint === lastCommittedPoint) {
                ...
                if (pointDistance(pointFrom(scenePointerX - rx, scenePointerY - ry), lastPoint) >= LINE_CONFIRM_THRESHOLD) {
                ...
                lastCommittedPoint &&
                pointDistance(pointFrom(scenePointerX - rx, scenePointerY - ry), lastCommittedPoint) < LINE_CONFIRM_THRESHOLD) {
                setCursor(this.interactiveCanvas, CURSOR_TYPE.POINTER);
                mutateElement(multiElement, {
        ...
        if (this.hitLinkElement &&
            ...
            if (hitElement &&
                ...
                setCursor(this.interactiveCanvas, isTextElement(hitElement) ? CURSOR_TYPE.TEXT : CURSOR_TYPE.CROSSHAIR);
            }
    ...
    handleEraser = (event, pointerDownState, scenePointer) => {
        ...
        const distance = pointDistance(pointFrom(pointerDownState.lastCoords.x, pointerDownState.lastCoords.y), pointFrom(scenePointer.x, scenePointer.y));
    ...
    handleCanvasPointerDown = (event) => {
        ...
        if (event.pointerType === "touch" &&
            ...
            this.updateScene({
                ...
                appState: {
                    ...
                    suggestedBindings: [],
                    selectedElementIds: makeNextSelectedElementIds(Object.keys(this.state.selectedElementIds)
        ...
        if (this.state.activeTool.type === "text") {
            ...
            const pendingImageElement = this.state.pendingImageElementId &&
    ...
    handleCanvasPointerUp = (event) => {
        ...
        const scenePointer = viewportCoordsToSceneCoords({ clientX: event.clientX, clientY: event.clientY }, this.state);
        ...
        if (this.hitLinkElement &&
            ...
            if (clicklength < 300 &&
                ...
                !isPointHittingLinkIcon(this.hitLinkElement, this.scene.getNonDeletedElementsMap(), this.state, pointFrom(scenePointer.x, scenePointer.y))) {
        ...
        if (this.state.activeTool.type === "selection") {
            ...
            if (pointerDownState.resize.handleType) {
                ...
                if (this.state.selectedLinearElement) {
                    ...
                    const ret = LinearElementEditor.handlePointerDown(event, this, this.store, pointerDownState.origin, linearElementEditor, this.scene);
                    if (ret.hitElement) {
                        ...
                        if (!someHitElementIsSelected &&
                            ...
                            this.setState((prevState) => {
                                ...
                                return {
                                    ...
                                    showHyperlinkPopup: hitElement.link || isEmbeddableElement(hitElement)
                                        ? "info"
                                        : false,
                                };
                            });
    ...
    insertIframeElement = ({ sceneX, sceneY, width, height, }) => {
        const [gridX, gridY] = getGridPoint(sceneX, sceneY, this.lastPointerDownEvent?.[KEYS.CTRL_OR_CMD]
            ? null
            : this.getEffectiveGridSize());
    ...
    handleLinearElementOnPointerDown = (event, elementType, pointerDownState) => {
        if (this.state.multiElement) {
            ...
            if (multiElement.points.length > 1 &&
                lastCommittedPoint &&
                pointDistance(pointFrom(pointerDownState.origin.x - rx, pointerDownState.origin.y - ry), lastCommittedPoint) < LINE_CONFIRM_THRESHOLD) {
                this.actionManager.executeAction(actionFinalize);
                return;
            }
            this.setState((prevState) => ({
            ...
            const [gridX, gridY] = getGridPoint(pointerDownState.origin.x, pointerDownState.origin.y, event[KEYS.CTRL_OR_CMD] ? null : this.getEffectiveGridSize());
            const topLayerFrame = this.getTopLayerFrameAtSceneCoords({
                x: gridX,
                y: gridY,
            });
    ...
    createGenericElementOnPointerDown = (elementType, pointerDownState) => {
        const [gridX, gridY] = getGridPoint(pointerDownState.origin.x, pointerDownState.origin.y, this.lastPointerDownEvent?.[KEYS.CTRL_OR_CMD]
            ? null
            : this.getEffectiveGridSize());
        const topLayerFrame = this.getTopLayerFrameAtSceneCoords({
            x: gridX,
        ...
        return withBatchedUpdatesThrottled((event) => {
            ...
            if (!pointerDownState.drag.hasOccurred &&
                (this.state.activeTool.type === "arrow" ||
                    this.state.activeTool.type === "line")) {
                if (pointDistance(pointFrom(pointerCoords.x, pointerCoords.y), pointFrom(pointerDownState.origin.x, pointerDownState.origin.y)) < DRAGGING_THRESHOLD) {
                    return;
                }
            }
            ...
            if (this.state.selectedLinearElement) {
                ...
                if (LinearElementEditor.shouldAddMidpoint(this.state.selectedLinearElement, pointerCoords, this.state, elementsMap)) {
                    const ret = LinearElementEditor.addMidpoint(this.state.selectedLinearElement, pointerCoords, this, !event[KEYS.CTRL_OR_CMD], elementsMap);
                    if (!ret) {
                        return;
                    }
                    // Since we are reading from previous state which is not possible with
                ...
                else if (linearElementEditor.pointerDownState.segmentMidpoint.value !== null &&
                    !linearElementEditor.pointerDownState.segmentMidpoint.added) {
                    return;
                }
                const didDrag = LinearElementEditor.handlePointDragging(event, this, pointerCoords.x, pointerCoords.y, (element, pointsSceneCoords) => {
            ...
            const isSelectingPointsInLineEditor = this.state.editingLinearElement &&
                event.shiftKey &&
                this.state.editingLinearElement.elementId ===
            ...
            if ((hasHitASelectedElement ||
                ...
                if (selectedElements.length > 0 &&
                    ...
                    if (event.altKey && !pointerDownState.hit.hasBeenDuplicated) {
                        ...
                        const hitElement = pointerDownState.hit.element;
                        const selectedElementIds = new Set(this.scene
                            .getSelectedElements({
                            selectedElementIds: this.state.selectedElementIds,
                            includeBoundTextElement: true,
                            includeElementsInFrames: true,
            ...
            if (this.state.selectionElement) {
                ...
                if (newElement.type === "freedraw") {
                    ...
                    if (shouldRotateWithDiscreteAngle(event) && points.length === 2) {
                        ({ width: dx, height: dy } = getLockedLinearCursorAlignSize(newElement.x, newElement.y, pointerCoords.x, pointerCoords.y));
                    }
                    if (points.length === 1) {
                        mutateElement(newElement, {
                            points: [...points, pointFrom(dx, dy)],
    ...
    onPointerUpFromPointerDownHandler(pointerDownState) {
        return withBatchedUpdates((childEvent) => {
            ...
            const { newElement, resizingElement, multiElement, activeTool, isResizing, isRotating, } = this.state;
            this.setState((prevState) => ({
                isResizing: false,
                isRotating: false,
            ...
            if (isLinearElement(newElement)) {
                ...
                if (!pointerDownState.drag.hasOccurred && newElement && !multiElement) {
                    mutateElement(newElement, {
                        points: [
                            ...newElement.points,
                            pointFrom(pointerCoords.x - newElement.x, pointerCoords.y - newElement.y),
                        ],
                    });
                    this.setState({
                    ...
                    if (isBindingEnabled(this.state) &&
                        ...
                        maybeBindLinearElement(newElement, this.state, pointerCoords, this.scene.getNonDeletedElementsMap(), this.scene.getNonDeletedElements());
                    }
            ...
            if (isEraserActive(this.state) && pointerStart && pointerEnd) {
                this.eraserTrail.endPath();
                const draggedDistance = pointDistance(pointFrom(pointerStart.clientX, pointerStart.clientY), pointFrom(pointerEnd.clientX, pointerEnd.clientY));
                    ...
                    if (this.state.selectedElementIds[hitElement.id]) {
                        if (isSelectedViaGroup(this.state, hitElement)) {
                            ...
                            this.setState((prevState) => {
                                ...
                                const newSelectedElements = getSelectedElements(this.scene.getNonDeletedElements(), { selectedElementIds: newSelectedElementIds });
                                return {
                                    ...selectGroupsForSelectedElements({
                                        editingGroupId: prevState.editingGroupId,
            ...
            if (!activeTool.locked && activeTool.type !== "freedraw" && newElement) {
                this.setState((prevState) => ({
                    ...
                    showHyperlinkPopup: isEmbeddableElement(newElement) && !newElement.link
                        ? "editor"
            ...
            if (hitElement &&
                ...
                isIframeLikeElement(hitElement) &&
                this.isIframeLikeElementCenter(hitElement, this.lastPointerUpEvent, pointerDownState.origin.x, pointerDownState.origin.y)) {
                this.handleEmbeddableCenterClick(hitElement);
            }
        });
    }
    ...
    initializeImage = async ({ imageFile, imageElement: _imageElement, showCursorImagePreview = false, }) => {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (this.state.pendingImageElementId !== imageElement.id &&
    ...
    onImageAction = async ({ insertOnCanvasDirectly, }) => {
        try {
    ...
    addNewImagesToImageCache = async (imageElements = getInitializedImageElements(this.scene.getNonDeletedElements()), files = this.files) => {
    ...
    maybeSuggestBindingsForLinearElementAtCoords = (linearElement, 
    /** scene coords */
    pointerCoords, 
    ...
    maybeDragNewGenericElement = (pointerDownState, event, informMutation = true) => {
        ...
        let [gridX, gridY] = getGridPoint(pointerCoords.x, pointerCoords.y, event[KEYS.CTRL_OR_CMD] ? null : this.getEffectiveGridSize());
        const image = isInitializedImageElement(newElement) &&
        ...
        const { snapOffset, snapLines } = snapNewElement(newElement, this, event, {
        ...
        if (this.state.activeTool.type === TOOL_TYPE.frame ||
            this.state.activeTool.type === TOOL_TYPE.magicframe) {
            this.setState({
                elementsToHighlight: getElementsInResizingFrame(this.scene.getNonDeletedElements(), newElement, this.state, this.scene.getNonDeletedElementsMap()),
            });
        }
    };
    maybeHandleResize = (pointerDownState, event) => {
        const selectedElements = this.scene.getSelectedElements(this.state);
        const selectedFrames = selectedElements.filter((element) => isFrameLikeElement(element));
        ...
        const pointerCoords = pointerDownState.lastCoords;
        let [resizeX, resizeY] = getGridPoint(pointerCoords.x - pointerDownState.resize.offset.x, pointerCoords.y - pointerDownState.resize.offset.y, event[KEYS.CTRL_OR_CMD] ? null : this.getEffectiveGridSize());
        ...
        if (!this.state.selectedElementsAreBeingDragged) {
            ...
            this.maybeCacheReferenceSnapPoints(event, selectedElements);
            const { snapOffset, snapLines } = snapResizingElements(selectedElements, getSelectedElements(originalElements, this.state), this, event, dragOffset, transformHandleType);
    ...
    handleWheel = withBatchedUpdates((event) => {
