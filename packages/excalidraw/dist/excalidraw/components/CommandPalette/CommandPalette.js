...
import { useApp, useAppProps, useExcalidrawActionManager, useExcalidrawSetAppState, } from "../App";
...
const CommandShortcutHint = ({ shortcut, className, children, }) => {
    ...
    return (_jsxs("div", { className: clsx("shortcut", className), children: [shortcuts.map((item, idx) => {
                return (_jsx("div", { className: "shortcut-wrapper", children: _jsx("div", { className: "shortcut-key", children: item === "$" ? "+" : item }) }, item));
            }), _jsx("div", { className: "shortcut-desc", children: children })] }));
};
...
export const CommandPalette = Object.assign((props) => {
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener(EVENT.KEYDOWN, commandPaletteShortcut, {
            capture: true,
        });
    }, [setAppState]);
    ...
    return _jsx(CommandPaletteInner, { ...props });
}, {
    defaultItems,
});
function CommandPaletteInner({ customCommandPaletteItems, }) {
    ...
    useEffect(() => {
        ...
        const getActionLabel = (action) => {
            ...
            if (action.label) {
                if (typeof action.label === "function") {
                    label = t(action.label(app.scene.getNonDeletedElements(), uiAppState, app));
                }
                else {
        ...
        if (uiAppState && app.scene && actionManager) {
            const elementsCommands = [
                ...
                actionLink,
            ].map((action) => actionToCommand(action, DEFAULT_CATEGORIES.elements, (command, action) => ({
                ...command,
                predicate: action.predicate
                    ? action.predicate
            ...
            setLastUsed(allCommands.find((command) => command.label === lastUsed?.label) ??
                null);
        }
    }, [
        stableDeps,
    ...
    const handleKeyDown = useStableCallback((event) => {
        const ignoreAlphanumerics = isWritableElement(event.target) ||
    ...
    return (_jsxs(Dialog, { onCloseRequest: () => closeCommandPalette(), closeOnClickOutside: true, title: false, size: 720, autofocus: true, className: "command-palette-dialog", children: [_jsx(TextField, { value: commandSearch, placeholder: t("commandPalette.search.placeholder"), onChange: (value) => {
                    setCommandSearch(value);
                }, selectOnRender: true, ref: inputRef }), !app.device.viewport.isMobile && (_jsxs("div", { className: "shortcuts-wrapper", children: [_jsx(CommandShortcutHint, { shortcut: "\u2191\u2193", children: t("commandPalette.shortcuts.select") }), _jsx(CommandShortcutHint, { shortcut: "\u21B5", children: t("commandPalette.shortcuts.confirm") }), _jsx(CommandShortcutHint, { shortcut: getShortcutKey("Esc"), children: t("commandPalette.shortcuts.close") })] })), _jsxs("div", { className: "commands", children: [lastUsed && !commandSearch && (_jsxs("div", { className: "command-category", children: [_jsxs("div", { className: "command-category-title", children: [t("commandPalette.recents"), _jsx("div", { className: "icon", style: {
                                            marginLeft: "6px",
                                        }, children: clockIcon })] }), _jsx(CommandItem, { command: lastUsed, isSelected: lastUsed.label === currentCommand?.label, onClick: (event) => executeCommand(lastUsed, event), disabled: !isCommandAvailable(lastUsed), onMouseMove: () => setCurrentCommand(lastUsed), showShortcut: !app.device.viewport.isMobile, appState: uiAppState })] })), Object.keys(commandsByCategory).length > 0 ? (Object.keys(commandsByCategory).map((category, idx) => {
                        return (_jsxs("div", { className: "command-category", children: [_jsx("div", { className: "command-category-title", children: category }), commandsByCategory[category].map((command) => (_jsx(CommandItem, { command: command, isSelected: command.label === currentCommand?.label, onClick: (event) => executeCommand(command, event), onMouseMove: () => setCurrentCommand(command), showShortcut: !app.device.viewport.isMobile, appState: uiAppState }, command.label)))] }, category));
                    })) : allCommands ? (_jsxs("div", { className: "no-match", children: [_jsx("div", { className: "icon", children: searchIcon }), " ", t("commandPalette.search.noMatch")] })) : null] })] }));
}
const CommandItem = ({ command, isSelected, disabled, onMouseMove, onClick, showShortcut, appState, }) => {
    const noop = () => { };
    return (_jsxs("div", { className: clsx("command-item", {
        ...
        }, onClick: disabled ? noop : onClick, onMouseMove: disabled ? noop : onMouseMove, title: disabled ? t("commandPalette.itemNotAvailable") : "", children: [_jsxs("div", { className: "name", children: [command.icon && (_jsx(InlineIcon, { icon: typeof command.icon === "function"
                            ? command.icon(appState)
                            : command.icon })), command.label] }), showShortcut && command.shortcut && (_jsx(CommandShortcutHint, { shortcut: command.shortcut }))] }));
};
