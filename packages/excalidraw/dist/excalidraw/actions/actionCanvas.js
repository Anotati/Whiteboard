...
export const actionChangeViewBackgroundColor = register({
    ...
    PanelComponent: ({ elements, appState, updateData, appProps }) => {
        ...
        return (_jsx(ColorPicker, { palette: null, topPicks: DEFAULT_CANVAS_BACKGROUND_PICKS, label: t("labels.canvasBackground"), type: "canvasBackground", color: appState.viewBackgroundColor, onChange: (color) => updateData({ viewBackgroundColor: color }), "data-testid": "canvas-background-picker", elements: elements, appState: appState, updateData: updateData }));
    },
});
...
export const zoomToFitBounds = ({ bounds, appState, canvasOffsets, fitToViewport = false, viewportZoomFactor = 1, minZoom = -Infinity, maxZoom = Infinity, }) => {
    ...
    const newZoomValue = getNormalizedZoom(clamp(roundToStep(adjustedZoomValue, ZOOM_STEP, "floor"), minZoom, maxZoom));
...
export const zoomToFit = ({ canvasOffsets, targetElements, appState, fitToViewport, viewportZoomFactor, minZoom, maxZoom, }) => {
...
export const actionZoomToFitSelection = register({
    ...
    keyTest: (event) => event.code === CODES.THREE &&
        event.shiftKey &&
        !event.altKey &&
        !event[KEYS.CTRL_OR_CMD],
});
