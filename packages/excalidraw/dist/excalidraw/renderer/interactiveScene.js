...
import { isElbowArrow, isFrameLikeElement, isLinearElement, isTextElement, } from "../element/typeChecks";
...
const _renderInteractiveScene = ({ canvas, elementsMap, visibleElements, selectedElements, allElementsMap, scale, appState, renderConfig, device, }) => {
    ...
    appState.searchMatches.forEach(({ id, focus, matchedLines }) => {
        ...
        if (element && isTextElement(element)) {
            const [elementX1, elementY1, , , cx, cy] = getElementAbsoluteCoords(element, elementsMap, true);
            context.save();
